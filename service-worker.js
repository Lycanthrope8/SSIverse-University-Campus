...
let _myAppName = "SSIverse_University_Campus";
...
let _EVERY_RESOURCE_FROM_CURRENT_LOCATION = [
  "^" + _escapeRegexSpecialCharacters(_getCurrentLocation()) + ".*",
];
let _EVERY_RESOURCE_FROM_CURRENT_ORIGIN = [
  "^" + _escapeRegexSpecialCharacters(_getCurrentOrigin()) + ".*",
];
...
async function _install() {
  if (_myRejectServiceWorkerOnLocalhost) {
    let rejectServiceWorker = _shouldResourceURLBeIncluded(
      _getCurrentLocation(),
      _LOCALHOST,
      _NO_LOCATION
    );
    if (rejectServiceWorker) {
      throw new Error(
        "The service worker is not allowed to be installed on the current location: " +
...
async function _fetchFromServiceWorker(request) {
  try {
    ...
    if (
      _myTryFetchFromCacheFirst ||
      _myForceTryFetchFromCacheFirstOnNetworkErrorEnabled
    ) {
      ...
      if (responseFromCache == null) {
        let ignoreURLParamsAsFallback = _shouldResourceURLBeIncluded(
          request.url,
          _IGNORE_INDEX_URL_PARAMS,
          _NO_RESOURCE
        );
        if (ignoreURLParamsAsFallback) {
          responseFromCache = await _fetchFromCache(
            request,
            ignoreURLParamsAsFallback
          );
          if (responseFromCache != null) {
            if (_myLogEnabled) {
              console.warn(
                "Try fetch from cache first using a fallback: " + request.url
              );
            }
          }
        }
    ...
    let [responseFromNetwork, responseHasBeenCached] =
      await _fetchFromNetworkAndPutInCache(request, true);
    if (
      _isResponseOk(responseFromNetwork) ||
      _isResponseOpaque(responseFromNetwork)
    ) {
      return responseFromNetwork;
    } else {
      if (!_myForceTryFetchFromCacheFirstOnNetworkErrorEnabled) {
        let enableForceTryFetchFromCacheFirstOnNetworkError =
          _myForceTryFetchFromCacheFirstOnNetworkErrorFromCurrentLocation &&
          _shouldResourceURLBeIncluded(
            request.url,
            _EVERY_RESOURCE_FROM_CURRENT_LOCATION,
            _NO_RESOURCE
          );
      ...
      let ignoreURLParamsAsFallback =
        _myFetchFromCacheIgnoringURLParamsForResourcesFromCurrentLocationAsFallback &&
        _shouldResourceURLBeIncluded(
          request.url,
          _EVERY_RESOURCE_FROM_CURRENT_LOCATION,
          _NO_RESOURCE
      ...
      if (ignoreURLParamsAsFallback) {
        let fallbackResponseFromCache = await _fetchFromCache(
          request,
          ignoreURLParamsAsFallback
        );
    ...
    let errorMessage =
      "An error occurred while trying to fetch from the service worker: " +
      request?.url;
    let responseFromServiceWorker = new Response(
      errorMessage + "\n\n" + error,
      {
        status: 500,
...
async function _fetchFromNetworkAndPutInCache(
  request,
  awaitOnlyFetchFromNetwork = false,
  useTempCache = false
) {
  ...
  if (awaitOnlyFetchFromNetwork) {
    ...
    responseHasBeenCached = await _postFetchFromNetwork(
      request,
      responseFromNetwork,
      useTempCache
    );
...
async function _fetchFromNetwork(request) {
  ...
  try {
    let fetchFromNetworkIgnoringBrowserCache =
      _myFetchFromNetworkIgnoringBrowserCacheForResourcesFromCurrentLocation &&
      _shouldResourceURLBeIncluded(
        request.url,
        _EVERY_RESOURCE_FROM_CURRENT_LOCATION,
        _NO_RESOURCE
    ...
    let errorMessage =
      "An error occurred while trying to fetch from the network: " +
      request.url;
...
async function _postFetchFromNetwork(
  request,
  responseFromNetwork,
  useTempCache = false
) {
  ...
  if (_shouldResourceBeCached(request, responseFromNetwork)) {
    responseHasBeenCached = await _putInCache(
      request,
      responseFromNetwork,
      useTempCache
    );
  } else if (
    _shouldDeleteFromCacheDueToOpaqueResponse(request, responseFromNetwork)
  ) {
    let ignoreURLParamsAsFallback =
      _myFetchFromCacheIgnoringURLParamsForResourcesFromCurrentLocationAsFallback &&
      _shouldResourceURLBeIncluded(
...
async function _fetchFromCache(request, ignoreURLParams = false) {
  ...
  try {
    ...
    if (hasCache) {
      ...
      responseFromCache = await currentCache.match(request, {
        ignoreSearch: ignoreURLParams,
      });
    }
  } catch (error) {
    ...
    if (_myLogEnabled) {
      console.error(
        "An error occurred while trying to get from the cache: " + request.url
      );
      console.error(error);
    }
  }
...
async function _putInCache(request, response, useTempCache = false) {
  ...
  try {
    ...
    if (_myLogEnabled) {
      console.error(
        "An error occurred while trying to put the response in the cache: " +
          request.url
      );
      console.error(error);
    }
...
async function _deleteFromCache(
  request,
  ...
  try {
    let currentCacheID = useTempCache ? _getTempCacheID() : _getCacheID();
    let currentCache = await caches.open(currentCacheID);
    deleteFromCacheSucceeded = await currentCache.delete(request, {
      ignoreSearch: ignoreURLParams,
    });
  } catch (error) {
    ...
    if (_myLogEnabled) {
      console.error(
        "An error occurred while trying to delete the resource from the cache: " +
          request.url
      );
      console.error(error);
    }
...
async function _cacheResourcesToPrecache() {
  ...
  let precacheResourceAsyncCallback =
    async function precacheResourceAsyncCallback(resourceFullURLToPrecache) {
      ...
      try {
        ...
        if (currentCache != null) {
          resourceAlreadyInCache =
            (await currentCache.match(resourceFullURLToPrecache, {
              ignoreVary: true,
            })) != null;
        }
        ...
        if (!resourceAlreadyInCache) {
          ...
          if (currentTempCache != null) {
            resourceAlreadyInTempCache =
              (await currentTempCache.match(resourceFullURLToPrecache, {
                ignoreVary: true,
              })) != null;
          }
        ...
        if (resourceHaveToBeCached) {
          let [responseFromNetwork, responseHasBeenCached] =
            await _fetchFromNetworkAndPutInCache(
              new Request(resourceFullURLToPrecache),
              false,
              true
            );
          resourceHasBeenPrecached =
            responseHasBeenCached != null && responseHasBeenCached;
        } else {
        ...
        if (_myLogEnabled) {
          console.error(
            "Failed to fetch the resource to precache: " +
              resourceFullURLToPrecache
          );
          console.error(error);
        }
      }
  ...
  for (let resourceURLToPrecache of _getResourceURLsToPrecache()) {
    ...
    promisesToAwait.push(
      precacheResourceAsyncCallback(resourceFullURLToPrecache)
    );
  }
  ...
  try {
    ...
    if (hasTempCache) {
      ...
      for (let currentTempCachedResourceRequest of currentTempCachedResourceRequests) {
        try {
          let currentTempCachedResource = await currentTempCache.match(
            currentTempCachedResourceRequest
          );
          await currentCache.put(
            currentTempCachedResourceRequest,
            currentTempCachedResource
          );
        } catch (error) {
          // Do nothing
        }
...
function _shouldResourceBeCached(request, response) {
  let cacheResource =
    !_myPutInCacheOnlyResourcesFromCurrentLocation ||
    _shouldResourceURLBeIncluded(
      request.url,
      _EVERY_RESOURCE_FROM_CURRENT_LOCATION,
      _NO_RESOURCE
...
function _getTempCacheID(
  cacheVersion = _myCacheVersion,
  serviceWorkerVersion = _myServiceWorkerVersion
) {
  return (
    _getCacheID(cacheVersion) + "_temp_v" + serviceWorkerVersion.toFixed(0)
  );
}

function _isCacheID(cacheID) {
  let matchCacheID = new RegExp(
    "^" + _escapeRegexSpecialCharacters(_myAppName) + "_cache_v\\d+$"
  );
  return cacheID.match(matchCacheID) != null;
}

function _isTempCacheID(tempCacheID) {
  let matchTempCacheID = new RegExp(
    "^" + _escapeRegexSpecialCharacters(_myAppName) + "_cache_v\\d+_temp_v\\d+$"
  );
...
function _shouldDeleteCacheID(cacheID) {
  ...
  if (validCacheID) {
    let cacheIDWithoutAppName = cacheID.replace(
      new RegExp("^" + _escapeRegexSpecialCharacters(_myAppName)),
      ""
    );

    let versions = Array.from(
      cacheIDWithoutAppName.matchAll(new RegExp("_v(\\d+)(?:_|$)", "g")),
      (match) => match[1]
    );
...
function _shouldDeleteTempCacheID(tempCacheID) {
  ...
  if (validTempCacheID) {
    let tempCacheIDWithoutAppName = tempCacheID.replace(
      new RegExp("^" + _escapeRegexSpecialCharacters(_myAppName)),
      ""
    );

    let versions = Array.from(
      tempCacheIDWithoutAppName.matchAll(new RegExp("_v(\\d+)(?:_|$)", "g")),
      (match) => match[1]
    );
    ...
    deleteTempCacheID =
      ...
      (parseInt(versions[0]) == _myCacheVersion &&
        parseInt(versions[1]) <= _myServiceWorkerVersion);
  }
...
function _getCurrentLocation(addTrailingSlash = true) {
  return (
    self.location.href.slice(0, self.location.href.lastIndexOf("/")) +
    (addTrailingSlash ? "/" : "")
  );
}
